        ...
        if (player !== null) {
            try {
                ...
                transaction (Connection.db) {
                    ...
                    val placeId = arena[Arenas.lobby] ?: throw PlayerException("§cA arena não possuí um lobby definido!")

                    val place = Places.select {
                        Places.id eq placeId
                    }.firstOrNull() ?: throw PlayerException("§cO lobby da arena não é válido!")
